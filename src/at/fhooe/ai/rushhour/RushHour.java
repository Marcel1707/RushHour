...
public class RushHour {
    ...
    public static void main(String argv[]) throws FileNotFoundException, IOException {
        ...
        float[][] relativeImprovements = new float[num_puzzles][num_heuristics];

        for (int i = 0; i < num_puzzles; i++) {
            int expandedZeroHeuristic = num_expanded[i][0];

            for (int j = 0; j < num_heuristics; j++) {
                int diff = expandedZeroHeuristic - num_expanded[i][j];
                relativeImprovements[i][j] = diff * 1.0f / expandedZeroHeuristic;
            }
        }
        ...
        for (int j = 0; j < num_heuristics; j++) {
            ...
            for (int i = 0; i < num_puzzles; i++) {
                improvementSum += relativeImprovements[i][j];
            }

            BigDecimal mean = BigDecimal.valueOf(improvementSum / num_puzzles * 100d).setScale(1, RoundingMode.HALF_UP);

            System.out.printf("heuristic %d relative improvement in expanded nodes (to zero heuristic): %s%%\n", j, mean);
        }
